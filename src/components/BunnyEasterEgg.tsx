...
export default function BunnyEasterEgg() {
  ...
  // Define the type for bunny state
  type BunnyStateType = typeof bunnyState;
  ...
  useEffect(() => {
    ...
    // Set showEasterEgg to true by default - we'll control visibility with CSS
  ...
  const isInViewport = () => {
    if (!hoverAreaRef.current) return false;
  ...
  const petBunny = () => {
    setBunnyState((prev: BunnyStateType) => {
 ...
 o(\")(\")`;
  ...
  return (
    <>
      {/* Large hover area to reveal bunny - covers about 1/4 of the screen */}
      <div
        ref={hoverAreaRef}
        className={`fixed transition-opacity duration-1000 z-10 ${
          showEasterEgg ? 'opacity-0' : 'hidden'
        }`}
        style={{
          left: `${position.x}%`,
          top: `${position.y}%`,
      ...
      >
        {/* Hint text that's slightly visible */}
        <div className="flex items-center justify-center w-full h-full">
          <div className="bg-gray-900/20 dark:bg-gray-100/20 text-white dark:text-black rounded-xl px-4 py-2">
            <p className="text-xs font-medium">Psst... there&apos;s a bunny here!</p>
          </div>
        </div>
      </div>
      ...
      <div
        ref={eggRef}
        className={`fixed transition-opacity duration-300 z-10 ${
          showEasterEgg && isHovering ? 'opacity-100' : 'opacity-0 pointer-events-none'
        }`}
        style={{
          left: `${position.x}%`,
          top: `${position.y}%`,
